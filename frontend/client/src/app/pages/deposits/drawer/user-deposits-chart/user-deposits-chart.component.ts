...
@Component({
  selector: 'user-deposits-chart',
  templateUrl: './user-deposits-chart.component.html',
  imports: [
    MatProgressSpinner,
...
export class UserDepositsChartComponent implements OnInit {
  ...
  chartOptionsS = toSignal(combineLatest([
    ...
    switchMap(([pubKey, timeUnit, recordType]) => this.#apiService.getUserDepositRecords(pubKey, new RecordFilter(timeUnit)).pipe(
    ...
    map((tuple) => {
      ...
      if (tuple.recordType == 'amount') {
        chartOptions.data[0].dataPoints = tuple.depositRecords.depositAmounts.map(record => ({x: new Date(record.timestamp), y: record.value}));
      } else {
        chartOptions.data[0].dataPoints = tuple.depositRecords.depositValues.map(record => ({x: new Date(record.timestamp), y: record.value}));
      }
      return chartOptions;
    }),
    delay(500),

...
export class Asset extends XAccount {
    ...
    static pubKeyFactory(bank: PublicKey, mint?: PublicKey): PublicKey {
        return PublicKey.findProgramAddressSync(
    ...
    static async from(publicKey: PublicKey): Promise<Asset> {
        const data = await PROGRAM.account.asset.fetch(publicKey);
        return new Asset(publicKey, data);
    }

    static async factory(bank: PublicKey, mint?: PublicKey): Promise<Asset> {
        const pubKey = Asset.pubKeyFactory(bank, mint);
        const accountInfo = await CONNECTION.getAccountInfo(pubKey);
        if (accountInfo) {
            return await this.from(pubKey);
        }
        return new Asset(pubKey, accountInfo);
    };
